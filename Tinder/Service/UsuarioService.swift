...
class UsuarioService {
    ...
    let usuarios:[Usuario] = [
        Usuario(id: 101, nome: "Marco", idade: 30, match: true, frase: "O ultimo a dar match chama",
        foto: "pessoa-1"),
        Usuario(id: 102, nome: "Xandz", idade: 27, match: true, frase: "Vamo joga Tibia", foto: "pessoa-2"
        ),
    ...
    func buscaUsuarios (completation: @escaping ([Usuario]?, Error?) -> ()) {
        
        DispatchQueue.main.asyncAfter(deadline: .now() + 0.0){
            completation(self.usuarios,nil)
        }
    }
}
